...
Deno.test('getProxyEnv: strips credentials from proxy URLs', () => {
	...
	assertEquals(result.proxies, [
		{ variable: 'HTTPS_PROXY', url: 'http://gluetun:8888', auth: true },
		{ variable: 'ALL_PROXY', url: 'socks5h://proxy:1080', auth: true }
	]);
...
Deno.test('getProxyEnv: reports proxies without credentials', () => {
	const result = getProxyEnv(envOf({ HTTP_PROXY: 'http://proxy:3128' }));
	assertEquals(result.proxies, [{ variable: 'HTTP_PROXY', url: 'http://proxy:3128', auth: false }]);
});
...
Deno.test('getProxyEnv: assumes http:// when the scheme is missing', () => {
	...
	assertEquals(result.proxies, [{ variable: 'HTTPS_PROXY', url: 'http://proxy:8080', auth: true }]);
});
...
Deno.test('getProxyEnv: prefers uppercase and falls back to lowercase', () => {
	...
	assertEquals(result.proxies, [
		{ variable: 'HTTPS_PROXY', url: 'http://upper:8080', auth: false },
		{ variable: 'http_proxy', url: 'http://lower:3128', auth: false }
	]);
});
...
Deno.test('getProxyEnv: never returns the raw value of an invalid URL', () => {
	...
	assertEquals(result.proxies, [{ variable: 'HTTPS_PROXY', url: null, auth: false }]);
