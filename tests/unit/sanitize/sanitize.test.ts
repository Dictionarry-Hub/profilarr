 ...
 *
 * Tests 1-3 cover the exact bypass techniques that the homegrown sanitizer missed.
...
Deno.test('sanitizeHtml: strips case-varied javascript: in href', () => {
	const input = '<a href="JaVaScRiPt:alert(1)">click</a>';
	const result = sanitizeHtml(input);
	assertEquals(result.includes('JaVaScRiPt:'), false, `Output still contains JaVaScRiPt: — ${result}`);
	assertEquals(result.includes('javascript:'), false, `Output still contains javascript: — ${result}`);
});
