...
function buildGroupMembersGuard(
  profileName: string,
  groupName: string,
  members: string[],
): string {
...
WHERE quality_profile_name = '${esc(profileName)}'
  ...
  if (nextMembers.length > 0) {
    const insertRows = nextMembers
      .map(
        (member, index) =>
          `SELECT '${esc(profileName)}' AS quality_profile_name, '${
            esc(groupName)
          }' AS quality_group_name, '${
            esc(member)
          }' AS quality_name, ${index} AS position`,
      )
      .join("\nUNION ALL\n");
...
async function queryCompiledGroupMembers(
  ...
  const { compile, getCache, databaseInstancesQueries, config, db } =
    await loadPcdModules();
  await config.init();
  await db.initialize();
...
function seedProfileWithGroup(
  ...
  opts.members.forEach((name, index) => {
    insertOp(dbPath, {
      ...
      state: "published",
      sql:
        `INSERT INTO qualities (name) SELECT '${name}' WHERE NOT EXISTS (SELECT 1 FROM qualities WHERE name = '${name}');`,
      sequence: 10 + index,
    });
  });
  ...
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profiles (name, description) VALUES ('${opts.profileName}', 'Test profile');`,
    sequence: 100,
  });
  ...
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_groups (quality_profile_name, name) VALUES ('${opts.profileName}', '${opts.groupName}');`,
    sequence: 110,
  });
  ...
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profile_qualities (quality_profile_name, quality_group_name, quality_name, position, enabled, upgrade_until) VALUES ('${opts.profileName}', '${opts.groupName}', NULL, 0, 1, 0);`,
    sequence: 200,
  });
...
setup(async () => {
  ...
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profiles (name, description) VALUES ('Conflict Profile', 'Test');`,
    sequence: 100,
  });
  insertOp(dbPath, {
    databaseId: askDbId,
    ...
    state: "published",
    sql:
      `INSERT INTO quality_groups (quality_profile_name, name) VALUES ('Conflict Profile', 'Conflict Group');`,
    sequence: 110,
  });
  askBaseMembers.forEach((member, index) => {
    insertOp(dbPath, {
      ...
      state: "published",
      sql:
        `INSERT INTO quality_group_members (quality_profile_name, quality_group_name, quality_name, position) VALUES ('Conflict Profile', 'Conflict Group', '${member}', ${index});`,
      sequence: 120 + index,
    });
  });
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profile_qualities (quality_profile_name, quality_group_name, quality_name, position, enabled, upgrade_until) VALUES ('Conflict Profile', 'Conflict Group', NULL, 0, 1, 0);`,
    sequence: 200,
  });
  ...
  askBaseMembers.forEach((member, index) => {
    insertOp(dbPath, {
      ...
      state: "published",
      sql:
        `DELETE FROM quality_group_members WHERE quality_profile_name = 'Conflict Profile' AND quality_group_name = 'Conflict Group' AND quality_name = '${member}';`,
      sequence: 300 + index,
    });
  });
  ...
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profiles (name, description) VALUES ('Add Profile', 'Test');`,
    sequence: 100,
  });
  insertOp(dbPath, {
    databaseId: addDbId,
    ...
    state: "published",
    sql:
      `INSERT INTO quality_groups (quality_profile_name, name) VALUES ('Add Profile', 'Add Group');`,
    sequence: 110,
  });
  ["HDTV-720p", "Bluray-720p"].forEach((member, index) => {
    insertOp(dbPath, {
      ...
      state: "published",
      sql:
        `INSERT INTO quality_group_members (quality_profile_name, quality_group_name, quality_name, position) VALUES ('Add Profile', 'Add Group', '${member}', ${index});`,
      sequence: 120 + index,
    });
  });
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profile_qualities (quality_profile_name, quality_group_name, quality_name, position, enabled, upgrade_until) VALUES ('Add Profile', 'Add Group', NULL, 0, 1, 0);`,
    sequence: 200,
  });
  ...
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profiles (name, description) VALUES ('Remove Profile', 'Test');`,
    sequence: 100,
  });
  insertOp(dbPath, {
    databaseId: removeDbId,
    ...
    state: "published",
    sql:
      `INSERT INTO quality_groups (quality_profile_name, name) VALUES ('Remove Profile', 'Remove Group');`,
    sequence: 110,
  });
  ["HDTV-720p", "WEBRip-720p", "Bluray-720p"].forEach((member, index) => {
    insertOp(dbPath, {
      ...
      state: "published",
      sql:
        `INSERT INTO quality_group_members (quality_profile_name, quality_group_name, quality_name, position) VALUES ('Remove Profile', 'Remove Group', '${member}', ${index});`,
      sequence: 120 + index,
    });
  });
  insertOp(dbPath, {
    ...
    state: "published",
    sql:
      `INSERT INTO quality_profile_qualities (quality_profile_name, quality_group_name, quality_name, position, enabled, upgrade_until) VALUES ('Remove Profile', 'Remove Group', NULL, 0, 1, 0);`,
    sequence: 200,
  });
...
test("basic: no errors in compile history", () => {
  ...
  const errors = history.filter((h) =>
    h.status === "error" || h.status === "conflicted" ||
    h.status === "conflicted_pending"
  );
  assertEquals(
    errors.length,
    0,
...
test("align: user reorder op is auto-dropped", () => {
  ...
  try {
    const op = db.prepare("SELECT state FROM pcd_ops WHERE id = ?").get(
      alignUserOpId,
    ) as {
      state: string;
    };
...
test("add: new member gets inserted at requested position", async () => {
  const members = await queryCompiledGroupMembers(
    addDbId,
    "Add Profile",
    "Add Group",
  );
