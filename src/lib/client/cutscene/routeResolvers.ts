...
export const routeResolvers: Record<string, () => Promise<string>> = {
	firstDatabaseChanges: async () => {
		const res = await fetch(resolve('/api/v1/databases'));
	...
	firstArrLibrary: async () => {
		const res = await fetch(resolve('/api/v1/arr'));
	...
	firstCustomFormatGeneral: async () => {
		const dbs = await (await fetch(resolve('/api/v1/databases'))).json();
		if (!dbs.length) return '/custom-formats';
		const { path } = await (
			await fetch(resolve(`/custom-formats/${dbs[0].id}/first/general`))
		).json();
		return path;
	},
	...
	firstQualityProfileGeneral: async () => {
		...
		const { path } = await (
			await fetch(resolve(`/quality-profiles/${dbs[0].id}/first/general`))
		).json();
		return path;
	},
	firstQualityProfileScoring: async () => {
		...
		if (!dbs.length) return '/quality-profiles';
		const { path } = await (
			await fetch(resolve(`/quality-profiles/${dbs[0].id}/first/scoring`))
		).json();
		return path;
	},
	firstQualityProfileQualities: async () => {
		...
		if (!dbs.length) return '/quality-profiles';
		const { path } = await (
			await fetch(resolve(`/quality-profiles/${dbs[0].id}/first/qualities`))
		).json();
		return path;
	},
	firstRegularExpression: async () => {
		...
		const { path } = await (await fetch(resolve(`/regular-expressions/${dbs[0].id}/first`))).json();
		return path;
	},
	firstDelayProfileGeneral: async () => {
		const dbs = await (await fetch(resolve('/api/v1/databases'))).json();
		if (!dbs.length) return '/delay-profiles';
		const { path } = await (await fetch(resolve(`/delay-profiles/${dbs[0].id}/first`))).json();
		return path;
	},
	firstNaming: async () => {
		const dbs = await (await fetch(resolve('/api/v1/databases'))).json();
		...
		const { path } = await (
			await fetch(resolve(`/media-management/${dbs[0].id}/naming/first`))
		).json();
		return path;
	},
	firstQualityDefinitions: async () => {
		...
		if (!dbs.length) return '/media-management';
		const { path } = await (
			await fetch(resolve(`/media-management/${dbs[0].id}/quality-definitions/first`))
		).json();
		return path;
	},
	firstMediaSettings: async () => {
		...
		if (!dbs.length) return '/media-management';
		const { path } = await (
			await fetch(resolve(`/media-management/${dbs[0].id}/media-settings/first`))
		).json();
		return path;
	}
};
