...
<div class="border-t border-border px-4 py-3">
	<div class="mb-2 text-xs font-medium text-text-soft">Add Custom Group</div>
...
{#if customGroups.length > 0}
	<div class="border-t border-border">
		{#each customGroups as group}
			<div
				class="group flex items-center justify-between gap-2 px-4 py-2 transition-colors hover:bg-surface-hover"
			>
				<button
					type="button"
				...
				<button
					...
					class="flex h-5 w-5 items-center justify-center rounded-control-sm text-text-subtle transition-colors hover:bg-danger-bg hover:text-danger-icon"
				>
					<X size={12} />
				</button>
			</div>
