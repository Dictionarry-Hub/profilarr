...
function sameRow(current: OrderedItem, next: OrderedItem): boolean {
  ...
  const currentMembers = (current.members ?? []).map((m) => m.name).filter(
    Boolean,
  );
...
WHERE NOT EXISTS (
  ...
  WHERE quality_profile_name = '${esc(profileName)}'
    ...
    const members = Array.from(
      new Set((item.members ?? []).map((m) => m.name).filter(Boolean)),
    );
...
WHERE quality_profile_name = '${esc(profileName)}'
  ...
  if (current.type === "group") {
    ...
    const nextMembers = (next.members ?? []).map((m) => m.name).filter(Boolean);
    const membersChanged = currentMembers.length !== nextMembers.length ||
      currentMembers.some((m, i) => m !== nextMembers[i]);

    if (membersChanged) {
      const membersGuard = buildGroupMembersGuard(
        profileName,
        current.name,
        currentMembers,
      );
...
export async function updateQualities(options: UpdateQualitiesOptions) {
  ...
  const upgradeUntilCount =
    input.orderedItems.filter((item) => item.upgradeUntil).length;
  ...
  const currentByKey = new Map(
    currentData.orderedItems.map((item) => [rowKey(item), item]),
  );
  const nextByKey = new Map(
    input.orderedItems.map((item) => [rowKey(item), item]),
  );

  const removedItems = currentData.orderedItems.filter((item) =>
    !nextByKey.has(rowKey(item))
  );
  const addedItems = input.orderedItems.filter((item) =>
    !currentByKey.has(rowKey(item))
  );
  ...
  for (const item of removedItems) {
    rowOps.push({
      description:
        `remove-quality-profile-row-${profileName}-${item.type}-${item.name}`,
      ...
      title:
        `Remove ${item.type} "${item.name}" from quality profile "${profileName}"`,
    });
  }
  ...
  for (const item of addedItems) {
    rowOps.push({
      description:
        `add-quality-profile-row-${profileName}-${item.type}-${item.name}`,
      ...
      title:
        `Add ${item.type} "${item.name}" to quality profile "${profileName}"`,
    });
  }
  ...
  for (const change of sortedUpdates) {
    const queries = buildUpdateQueries(
      profileName,
      change.current,
      change.next,
    );
    ...
    rowOps.push({
      description:
        `update-quality-profile-row-${profileName}-${change.next.type}-${change.next.name}`,
      queries,
      desiredState: rowDesiredState(
        "update",
        rowKey(change.next),
        change.current,
        change.next,
      ...
      title:
        `Update ${change.next.type} "${change.next.name}" on quality profile "${profileName}"`,
    });
  }
