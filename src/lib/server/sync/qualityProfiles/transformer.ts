...
export function transformQualityProfile(
  ...
  for (const item of profile.qualities) {
    if (item.type === "group" && item.members) {
      for (const member of item.members) {
        const apiName = qualityApiMappings.get(member.name.toLowerCase()) ??
  ...
  for (const item of profile.qualities) {
    if (item.type === "group") {
      ...
      // Single quality
      const apiName = qualityApiMappings.get(item.name.toLowerCase()) ??
        mapQualityName(item.name, arrType);
      const quality = allQualities[apiName];
  ...
  for (const [qualityName, quality] of Object.entries(allQualities)) {
    if (
      !usedQualityNames.has(qualityName.toUpperCase()) &&
      !qualityIdsInGroups.has(quality.id)
    ) {
      items.push({
        quality: { ...quality },
        items: [],
...
export async function fetchQualityProfileFromPcd(
  ...
  const allQualities = await db.selectFrom("qualities").select(["id", "name"])
    .execute();
  ...
  const groupsMap = new Map<
    string,
    { name: string; members: { id: number; name: string }[] }
  >();
  ...
  const cfScores = await db
    ...
    .where((eb) =>
      eb.or([
        eb("qpcf.arr_type", "=", arrType),
        eb("qpcf.arr_type", "=", "all"),
      ])
    )
    .execute();
